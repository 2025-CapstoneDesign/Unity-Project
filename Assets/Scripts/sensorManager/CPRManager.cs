...
public class CPRManager
{
    ...
    private const int requiredCompressions = 30;

    public bool IsCPRPassed { get; private set; } = false;

    // 생성자에서 SensorManager를 주입받고 즉시 실행
    public CPRManager(SensorManager manager)
    {
        sensorManager = manager ?? throw new ArgumentNullException(nameof(manager)); // Null 방지
        Debug.Log("CPRManager 초기화");
        Initialize();
    ...
    private void HandleSensorData(SensorType sensorType, double data)
    {
        ...
        if (sensorType == SensorType.CompressionSensor)
        {
            ...
            if (data >= 60 && data <= 100)
            {
                ...
                {
                    Debug.Log("30회가 넘었습니다!");
                    StopCPR();
                    IsCPRPassed = true; // CPR 통과
                }
            }
            else
            {
